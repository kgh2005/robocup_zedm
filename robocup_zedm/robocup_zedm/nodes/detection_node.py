...
class DetectionNode(Node):
    def __init__(self):
        ...
        # ==========
        self.declare_parameter("topic.rgb_topic", "/zed/zed_node/rgb/image_rect_color")
        self.declare_parameter("topic.bbox_topic", "/Bounding_box")
        self.rgb_topic = self.get_parameter("topic.rgb_topic").value
        self.bbox_topic = self.get_parameter("topic.bbox_topic").value
        # ==========
        self.declare_parameter("yolo.device", "0")             # "0" / "cpu"
        ...
        self.imgsz = int(self.get_parameter("yolo.imgsz").value)
        self.half = bool(self.get_parameter("yolo.half").value)
        # ==========
        # 디버그: imshow는 기본 OFF 권장
        self.declare_parameter("show_imshow", False)
        self.declare_parameter("debug_topic", "/debug_image")
        self.declare_parameter("publish_debug", True)
        self.declare_parameter("debug_rate_hz", 15.0)
        ...
        self.sub = self.create_subscription(Image, self.rgb_topic, self.image_callback, qos_profile_sensor_data)
        ...
        self.timer = self.create_timer(period, self.processing)
